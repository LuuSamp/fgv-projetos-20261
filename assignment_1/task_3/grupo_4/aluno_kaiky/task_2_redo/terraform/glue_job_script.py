...
def main():
    ...
    # --- Write outputs (Parquet + Glue Data Catalog for Athena) ---
    base_path = f"s3://{s3_bucket}/{out_prefix}"

    def _write_table(df, name: str):
        path = f"{base_path}/{name}/"
        dyf = DynamicFrame.fromDF(df.repartition(1), glue_context, name)
        ...
        sink.setCatalogInfo(catalogDatabase=glue_database, catalogTableName=name)
        sink.setFormat("glueparquet")
        sink.writeFrame(dyf)

    _write_table(dim_customers, "dim_customers")
    _write_table(dim_products, "dim_products")
    _write_table(dim_dates, "dim_dates")
    _write_table(dim_countries, "dim_countries")
    _write_table(fact_orders, "fact_orders")
