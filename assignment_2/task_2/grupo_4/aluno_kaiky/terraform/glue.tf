resource "aws_glue_connection" "mysql" {
  name            = "${local.name_prefix}-mysql-conn"
  connection_type = "JDBC"

  connection_properties = {
    JDBC_CONNECTION_URL = "jdbc:mysql://${local.rds_endpoint}:${local.rds_port}/${var.db_name}"
    USERNAME            = var.db_user
    PASSWORD            = var.db_password
  }

  physical_connection_requirements {
    availability_zone      = data.aws_subnet.glue.availability_zone
    security_group_id_list = local.glue_connection_sg_ids
    subnet_id              = local.glue_connection_subnet_id
  }

  tags = local.tags
}

data "archive_file" "glue_modules" {
  type        = "zip"
  source_dir  = abspath("${path.module}/../glue_jobs")
  output_path = "${path.module}/build/glue_modules.zip"
  excludes    = ["etl_job.py"]
}

resource "aws_s3_object" "glue_entry_script" {
  bucket       = aws_s3_bucket.etl.id
  key          = "${local.scripts_prefix}/etl_job.py"
  source       = abspath("${path.module}/../glue_jobs/etl_job.py")
  source_hash  = filemd5(abspath("${path.module}/../glue_jobs/etl_job.py"))
  content_type = "text/x-python"

  depends_on = [
    aws_s3_bucket_ownership_controls.etl,
    aws_s3_bucket_public_access_block.etl,
  ]
}

resource "aws_s3_object" "glue_modules_zip" {
  bucket       = aws_s3_bucket.etl.id
  key          = "${local.scripts_prefix}/glue_modules.zip"
  source       = data.archive_file.glue_modules.output_path
  source_hash  = data.archive_file.glue_modules.output_md5
  content_type = "application/zip"

  depends_on = [
    aws_s3_bucket_ownership_controls.etl,
    aws_s3_bucket_public_access_block.etl,
  ]
}

resource "aws_glue_job" "etl" {
  name     = "${local.name_prefix}-job"
  role_arn = data.aws_iam_role.glue.arn

  glue_version      = "4.0"
  worker_type       = "G.1X"
  number_of_workers = 2

  command {
    name            = "glueetl"
    python_version  = "3"
    script_location = "s3://${aws_s3_bucket.etl.bucket}/${aws_s3_object.glue_entry_script.key}"
  }

  default_arguments = {
    "--job-language"                     = "python"
    "--extra-py-files"                   = "s3://${aws_s3_bucket.etl.bucket}/${aws_s3_object.glue_modules_zip.key}"
    "--enable-continuous-cloudwatch-log" = "true"
    "--enable-glue-datacatalog"          = "true"
    "--enable-metrics"                   = "true"

    "--rds_endpoint"    = local.rds_endpoint
    "--rds_port"        = tostring(local.rds_port)
    "--db_name"         = var.db_name
    "--db_user"         = var.db_user
    "--db_password"     = var.db_password
    "--s3_bucket"       = aws_s3_bucket.etl.bucket
    "--out_prefix"      = local.analytics_prefix
    "--glue_database"   = aws_glue_catalog_database.star.name
    "--pipeline_name"   = local.pipeline_name
    "--watermark_table" = local.watermark_table
  }

  connections = [aws_glue_connection.mysql.name]

  depends_on = [
    aws_glue_catalog_database.star,
    aws_glue_catalog_table.fact_orders,
    aws_glue_catalog_table.dim_customers,
    aws_glue_catalog_table.dim_products,
    aws_glue_catalog_table.dim_dates,
    aws_glue_catalog_table.dim_countries,
    aws_s3_object.glue_entry_script,
    aws_s3_object.glue_modules_zip,
  ]
  tags = local.tags
}
