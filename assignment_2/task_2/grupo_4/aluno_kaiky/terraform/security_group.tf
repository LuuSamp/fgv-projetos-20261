resource "aws_security_group" "glue_connection" {
  name        = "${local.name_prefix}-glue-conn-sg"
  description = "Glue Connection SG (self-ingress all ports, egress all)"
  vpc_id      = local.vpc_id
  tags        = local.tags
}

# Glue requirement (in some labs): at least one SG must allow all inbound ports.
# Restricting source to itself limits the exposure to ENIs within this SG.
resource "aws_security_group_rule" "glue_self_ingress_all" {
  type              = "ingress"
  security_group_id = aws_security_group.glue_connection.id

  from_port = 0
  to_port   = 0
  protocol  = "-1"

  self = true
}

resource "aws_security_group_rule" "glue_egress_all" {
  type              = "egress"
  security_group_id = aws_security_group.glue_connection.id

  from_port   = 0
  to_port     = 0
  protocol    = "-1"
  cidr_blocks = ["0.0.0.0/0"]
}

# Allow Glue connection ENIs to reach MySQL on the RDS security group.
resource "aws_security_group_rule" "rds_mysql_from_glue" {
  count = local.glue_sg_id_resolved == null ? 0 : 1

  type                     = "ingress"
  security_group_id        = local.glue_sg_id_resolved
  source_security_group_id = aws_security_group.glue_connection.id
  from_port                = 3306
  to_port                  = 3306
  protocol                 = "tcp"
  description              = "MySQL from Glue connection SG (Task 2 incremental ETL)"
}

locals {
  # Always include the "self-ingress all ports" SG to satisfy Glue validation.
  # Include RDS SG when resolved (same as var.glue_sg_id default from discovery).
  glue_connection_sg_ids = local.glue_sg_id_resolved == null ? [aws_security_group.glue_connection.id] : distinct([
    aws_security_group.glue_connection.id,
    local.glue_sg_id_resolved,
  ])
}

